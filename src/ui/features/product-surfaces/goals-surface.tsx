        ...
        data.mode === "current_path" ? (
          ...
          <>
            <header className="fy-surface-heading">
              <p>{data.run.hypotheticalLabel}</p>
              <h1>{data.run.label}</h1>
              <span>{data.run.classificationLabel}</span>
            </header>
            {data.warning ? (
            ...
            <section className="fy-goals-list is-preview" aria-label="Financial goals">
              {data.goals.map((goal) => (
